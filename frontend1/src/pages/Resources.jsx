...
export function Resources() {
  ...
  const handleSubmitNewResource = async (e) => {
    ...
    try {
      ...
      const savedResource = await backendService.createResourceRequest(resourceToAdd);
  ...
  return (
    <div className={`p-6 max-w-7xl mx-auto ${isDarkMode ? 'text-gray-100' : 'text-gray-900'}`}>
      <div className="flex justify-between items-center mb-6">
        <h1 className={`text-2xl font-bold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Resource Management</h1>
        <button 
      ...
      <div className={`rounded-lg shadow overflow-hidden ${isDarkMode ? 'bg-gray-800' : 'bg-white'}`}>
        <div className="min-w-full divide-y divide-gray-200">
          ...
          <div className={`divide-y ${isDarkMode ? 'divide-gray-700' : 'divide-gray-200'}`}>
            {isLoading ? (
              <div className={`px-6 py-4 text-center ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Loading resources...</div>
            ) : filteredResources.length === 0 ? (
              <div className={`px-6 py-4 text-center ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>No resources found</div>
            ) : (
              filteredResources.map((resource) => (
                <div key={resource.id} className={`grid grid-cols-6 px-6 py-4 items-center ${
                  ...
                  <div className="col-span-2">
                    <div className={`font-medium ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>{resource.name}</div>
                    <div className={`text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>{resource.category}</div>
                  </div>
                  <div className={`${isDarkMode ? 'text-gray-300' : 'text-gray-900'}`}>
                    {resource.quantity} {resource.unit}
                  </div>
                  <div className={`${isDarkMode ? 'text-gray-300' : 'text-gray-900'}`}>{resource.location}</div>
                  <div>
      ...
      {isModalOpen && selectedResource && (
        ...
        >
          ...
          >
            <div className="flex justify-between items-center mb-6">
              <div>
                <h3 className={`text-xl font-semibold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>{selectedResource.name}</h3>
                <p className={`text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>{selectedResource.category}</p>
              </div>
              <button
                onClick={handleCloseModal}
            ...
            <div className="space-y-5">
              <div className="grid grid-cols-2 gap-4">
                <div className={`flex items-center space-x-2 p-3 rounded-lg ${
                  ...
                  <div>
                    <p className={`text-xs ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Urgency</p>
                    <p className={`text-sm font-medium ${isDarkMode ? 'text-white' : 'text-gray-900'}`} style={{ textTransform: 'capitalize' }}>{selectedResource.urgency}</p>
                  </div>
                </div>
                ...
                <div className={`flex items-center space-x-2 p-3 rounded-lg ${
                  ...
                  <div>
                    <p className={`text-xs ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Quantity</p>
                    <p className={`text-sm font-medium ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>{selectedResource.quantity} {selectedResource.unit}</p>
                  </div>
                </div>
              </div>
              ...
              <div className={`flex items-center space-x-2 p-3 rounded-lg ${
                ...
                <div>
                  ...
                  <p className={`text-sm font-medium ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>{selectedResource.location}</p>
                </div>
              </div>
              ...
              <div className={`flex items-center space-x-2 p-3 rounded-lg ${
                ...
                <div>
                  ...
                  <p className={`text-sm font-medium ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>{new Date(selectedResource.timestamp).toLocaleString()}</p>
                </div>
              </div>
              ...
              <div className={`p-3 rounded-lg ${
                ...
                <p className={`text-sm ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>{selectedResource.description}</p>
              </div>
              ...
              <div className={`border-t ${isDarkMode ? 'border-gray-700' : 'border-gray-200'} pt-5`}>
                <h4 className={`text-sm font-medium ${isDarkMode ? 'text-white' : 'text-gray-900'} mb-3`}>
                  {selectedResource.status === 'available' ? 'Provided By' : 'Requested By'}
                </h4>
                <div className="space-y-3">
                  <div className={`flex items-center space-x-2 p-3 rounded-lg ${
                    ...
                    <div>
                      ...
                      <p className={`text-sm font-medium ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>{selectedResource.providedBy.name}</p>
                    </div>
                  </div>
                  <div className={`flex items-center space-x-2 p-3 rounded-lg ${
                    isDarkMode ? 'bg-gray-700' : 'bg-gray-50'
                    ...
                    <div>
                      ...
                      <p className={`text-sm font-medium ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>{selectedResource.providedBy.email}</p>
                    </div>
                  </div>
                  <div className={`flex items-center space-x-2 p-3 rounded-lg ${
                    isDarkMode ? 'bg-gray-700' : 'bg-gray-50'
                    ...
                    <div>
                      ...
                      <p className={`text-sm font-medium ${isDarkMode ? 'text-white' : 'text-gray-900'}`} style={{ textTransform: 'capitalize' }}>{selectedResource.providedBy.role}</p>
                    </div>
                  </div>
                </div>
              </div>
      ...
      {isAddResourceModalOpen && (
        ...
        >
          ...
          >
            <div className="flex justify-between items-center mb-6">
              <div>
                <h3 className={`text-xl font-semibold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Request New Resource</h3>
                <p className={`text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-500'} mt-1`}>Fill in the details to request a new resource</p>
              </div>
              <button
                onClick={handleCloseAddResourceModal}
                className={`${isDarkMode ? 'text-gray-400 hover:text-gray-300' : 'text-gray-400 hover:text-gray-500'} transition-colors`}
            ...
            <form onSubmit={handleSubmitNewResource} className="space-y-4">
              ...
              <div>
                ...
                <div className="relative">
                  <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                    <MapPin className={`h-5 w-5 ${isDarkMode ? 'text-gray-400' : 'text-gray-400'}`} />
                  </div>
                  <Autocomplete
                    ...
                    options={{
                      types: ['establishment', 'school', 'hospital', 'university', 'point_of_interest'],
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setNewResource(prev => ({ ...prev, urgency: e.target.value }))}
                  className={`block w-full px-3 py-2 border rounded-md shadow-sm focus:outline-none focus:ring-red-500 focus:border-red-500 sm:text-sm ${
                    isDarkMode 
                      ? 'bg-gray-700 border-gray-600 text-white' 
                      : 'bg-white border-gray-300 text-gray-900'
